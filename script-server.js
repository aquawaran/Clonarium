...
function showScreen(screenName) {
    ...
    // Показать нужный экран и активировать соответствующую кнопку
    switch(screenName) {
        case 'feed':
            document.getElementById('feedScreen').classList.add('active');
            const feedBtn = document.querySelector('.bottom-nav-item[data-screen="feed"]');
            if (feedBtn) feedBtn.classList.add('active');
            loadPosts();
            break;
        case 'profile':
            document.getElementById('profileScreen').classList.add('active');
            const profileBtn = document.querySelector('.bottom-nav-item[data-screen="profile"]');
...
async function createPostFromProfile(content) {
    const formData = new FormData();
    formData.append('content', content);
