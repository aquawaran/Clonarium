...
function getCommentAvatarHtml(comment) {
    const fallbackChar = (comment?.authorName || '').trim().charAt(0).toUpperCase() || '😊';

    if (comment?.authorAvatar) {
        return `
            <img src="${comment.authorAvatar}" alt="${fallbackChar}" onerror="this.style.display='none'; var fallback = this.nextElementSibling; if (fallback) { fallback.classList.add('visible'); }">
            <span class="comment-avatar-fallback">${fallbackChar}</span>
        `;
    }

    return `<span class="comment-avatar-fallback visible">${fallbackChar}</span>`;
}
